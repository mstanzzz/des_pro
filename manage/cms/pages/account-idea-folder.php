...
$page_title = "Editing: account-idea-folder page";
$page_group = "account";
$page = "account";
$ts = time();
...
$sql = "SELECT account_idea_folder_id FROM account_idea_folder WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
$result = $dbCustom->getResult($db,$sql);
if($result->num_rows == 0){
	$sql = "INSERT INTO account_idea_folder
		(profile_account_id) 
		VALUES ('".$_SESSION['profile_account_id']."')"; 
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['account_idea_folder_id'] = $db->insert_id;
}else{
	$_SESSION['account_idea_folder_id'] = (isset($_REQUEST['account_idea_folder_id'])) ? $_REQUEST['account_idea_folder_id'] : 0;
}
if(!is_numeric($_SESSION['account_idea_folder_id'])) $_SESSION['account_idea_folder_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE account_idea_folder
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
	...
	if(!$stmt->bind_param("sssssssssssssssssssssi"
						...
						,$p_9_head  
						,$p_9_text
						,$_SESSION['account_idea_folder_id'])){
...
$sql = "SELECT *
FROM account_idea_folder 
WHERE account_idea_folder_id = '".$_SESSION['account_idea_folder_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<form name="form" action="account-idea-folder.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="account_idea_folder_id" value="<?php echo $_SESSION['account_idea_folder_id']; ?>">
