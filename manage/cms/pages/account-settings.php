...
$page_title = "Editing: account_settings page";
$page_group = "account";
$page = "account";
$ts = time();
...
$sql = "SELECT account_settings_id FROM account_settings WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
...
if($result->num_rows == 0){
	...
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['account_settings_id'] = $db->insert_id;
}else{
	$_SESSION['account_settings_id'] = (isset($_REQUEST['account_settings_id'])) ? $_REQUEST['account_settings_id'] : 0;
}
if(!is_numeric($_SESSION['account_settings_id'])) $_SESSION['account_settings_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE account_settings
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
...
WHERE account_settings_id = '".$_SESSION['account_settings_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<form name="form" action="account-settings.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="account_settings_id" value="<?php echo $_SESSION['account_settings_id']; ?>">
