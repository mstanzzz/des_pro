...
$page_title = "Editing: Locations";
$page_group = "locations";
$page = "locations";
$ts = time();

$db = $dbCustom->getDbConnect(SITE_N_DATABASE);

$sql = "SELECT locations_id FROM locations WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
...
if($result->num_rows == 0){
	...
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['locations_id'] = $db->insert_id;
}else{
	$_SESSION['locations_id'] = (isset($_REQUEST['locations_id'])) ? $_REQUEST['locations_id'] : 0;
}
if(!is_numeric($_SESSION['locations_id'])) $_SESSION['locations_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE locations
						SET p_1_head = ?
						,p_1_text = ? 												
						,p_2_head = ?
						,p_2_text = ? 						
...
$sql = "SELECT *
FROM locations 
WHERE locations_id = '".$_SESSION['locations_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<center><h1>Locations Page</h1></center>
<form name="form" action="locations.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="locations_id" value="<?php echo $_SESSION['locations_id']; ?>">
