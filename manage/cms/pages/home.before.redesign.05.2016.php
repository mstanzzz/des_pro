...
*/
	...
	$upper_right_head_content_type = (isset($_POST['upper_right_head_content_type'])) ? trim(addslashes($_POST['upper_right_head_content_type'])) : '';

	$upper_right_head_text = (isset($_POST['upper_right_head_text'])) ? trim(addslashes($_POST['upper_right_head_text'])) : '';
 ...
 $page	= 'home';
	...
	*/	
		//if(in_array(2,$user_functions_array)){
			...
			,banner_rotate = '%u'
			,banner_random = '%u'
			,upper_right_head_content_type = '%s'
			,img_2_id = '%u'
...
//echo $del_submit_button_id;
	...
	if($result->num_rows > 0){
		...
		$upper_right_head_content_type = $object->upper_right_head_content_type;
		...
		$banner_random = 1;
		$img_2_id = 0;
		$upper_right_head_content_type = 'text';
	...
	if(!isset($_SESSION['temp_page_fields']['upper_right_head_content_type'])) $_SESSION['temp_page_fields']['upper_right_head_content_type'] = $upper_right_head_content_type;	
...
function get_query_str(){
	...
	query_str += "&upper_right_head_content_type="+document.form.upper_right_head_content_type.value; 

	query_str += "&upper_right_head_text="+escape(tinyMCE.get('upper_right_head_text').getContent());
	
	
	//alert(escape(tinyMCE.get('upper_right_head_text').getContent()));
	

	query_str +=  "&upper_right_content_type="+$("input:radio[name='upper_right_content_type']:checked").val();
     		...
     		<div class="page_actions edit_page">
                        ...
                        <table cellpadding="10" cellspacing="0">
							<thead>
								...
								<?php
									...
									//image
									$block .= "<td valign='top'><img src='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/cms/banner/small/".$row->file_name."' 
									width='90%' /></td>";
									//image name
                ...
                <fieldset class="edit_content">
					...
					?>
                    	...
                    	<div style="float:left; margin-left:50px;">                    
						<a href="<?php echo SITEROOT;?>/manage/cms/pages/set-home-cat-display-order.php?list_type=cart" 
					...
					?>
                    	...
                    	<div style="float:left; margin-left:50px;">                    
						<a href="<?php echo SITEROOT;?>/manage/cms/pages/set-home-cat-display-order.php?list_type=showroom" 
                        class="btn btn-primary btn-small fancybox fancybox.iframe">Set Display Order</a>
						</div>
					...
					if($_SESSION['temp_page_fields']['content_heading_cat_id']){
						echo SITEROOT.'/'.$_SESSION['global_url_word'].getUrlByCatId($_SESSION['temp_page_fields']['content_heading_cat_id']);	
					}elseif($_SESSION['temp_page_fields']['content_heading_url'] != ''){
						echo SITEROOT.'/'.$_SESSION['global_url_word'].$_SESSION['temp_page_fields']['content_heading_url'];	
					}else{						
						$page_name = getURLFileNameById($_SESSION['temp_page_fields']['content_heading_page_seo_id']);
						if($page_name == 'app'){
							echo SITEROOT.'/app/';					
						}else{
							echo SITEROOT.'/'.$_SESSION['global_url_word'].$page_name.'.html';
						}
					}
				...
				<fieldset class="edit_content">
					...
					<div class="colcontainer">
                        ...
                        <div class="twocols">
							...
							<select name="upper_right_head_content_type">
                            <option value="img" <?php if($_SESSION['temp_page_fields']['upper_right_head_content_type'] == 'img') echo 'selected'; ?>>Image</option>
                            <option value="text" <?php if($_SESSION['temp_page_fields']['upper_right_head_content_type'] == 'text') echo 'selected'; ?>>Text</option>
                            </select>
                        </div>
                    </div>
					...
					$result = $dbCustom->getResult($db,$sql);							
							if($result->num_rows > 0){
								$img_obj = $result->fetch_object();
								echo "<img width='200' src='".SITEROOT."/ul_cms/".$domain."/".$img_obj->file_name."' />";
							}else{
								echo "no image";	
							}
							?>
					...
					<?php
						...
						if($_SESSION['temp_page_fields']['link_cat_id']){
							echo SITEROOT.'/'.$_SESSION['global_url_word'].getUrlByCatId($_SESSION['temp_page_fields']['link_cat_id']);	
						}elseif($_SESSION['temp_page_fields']['link_url'] != ''){
							echo SITEROOT.'/'.$_SESSION['global_url_word'].$_SESSION['temp_page_fields']['link_url'];	
						}else{
				...
				<fieldset class="edit_content">
					...
					<?php
						...
						if($_SESSION['temp_page_fields']['link_1_cat_id']){
							echo SITEROOT.'/'.$_SESSION['global_url_word'].getUrlByCatId($_SESSION['temp_page_fields']['link_1_cat_id']);	
						}elseif($_SESSION['temp_page_fields']['link_1_url'] != ''){
							echo SITEROOT.'/'.$_SESSION['global_url_word'].$_SESSION['temp_page_fields']['link_1_url'];	
						}else{
