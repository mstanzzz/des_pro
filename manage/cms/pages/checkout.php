...
$sql = "SELECT checkout_id FROM checkout WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
...
if($result->num_rows == 0){
	...
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['checkout_id'] = $db->insert_id;
}else{
	$_SESSION['checkout_id'] = (isset($_REQUEST['checkout_id'])) ? $_REQUEST['checkout_id'] : 0;
}
if(!is_numeric($_SESSION['checkout_id'])) $_SESSION['checkout_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE checkout
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
...
$sql = "SELECT *
FROM checkout 
WHERE checkout_id = '".$_SESSION['checkout_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<form name="form" action="checkout.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="checkout_id" value="<?php echo $_SESSION['checkout_id']; ?>">
