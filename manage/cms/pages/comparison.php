...
$sql = "SELECT comparison_id FROM comparison WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
...
if($result->num_rows == 0){
	...
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['comparison_id'] = $db->insert_id;
}else{
	$_SESSION['comparison_id'] = (isset($_REQUEST['comparison_id'])) ? $_REQUEST['comparison_id'] : 0;
}
if(!is_numeric($_SESSION['comparison_id'])) $_SESSION['comparison_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE comparison
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
...
$sql = "SELECT *
FROM comparison 
WHERE comparison_id = '".$_SESSION['comparison_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<center><h1>Comparison Page</h1></center>	

<form name="form" action="comparison.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="comparison_id" value="<?php echo $_SESSION['comparison_id']; ?>">
