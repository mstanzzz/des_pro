...
$module = new Module;
$page_title = "Editing: video_library_detail";
$page_group = "video=-library-detail";
$page = "video-library-detail";
$ts = time();
$db = $dbCustom->getDbConnect(SITE_N_DATABASE);
$sql = "SELECT video_library_detail_id FROM video_library_detail WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
$result = $dbCustom->getResult($db,$sql);
if($result->num_rows == 0){
	$sql = "INSERT INTO video_library_detail 
		(profile_account_id) 
		VALUES ('".$_SESSION['profile_account_id']."')"; 
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['video_library_detail_id'] = $db->insert_id;
}else{
	$_SESSION['video_library_detail_id'] = (isset($_REQUEST['video_library_detail_id'])) ? $_REQUEST['video_library_detail_id'] : 0;
}
if(!is_numeric($_SESSION['video_library_detail_id'])) $_SESSION['video_library_detail_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE video_library_detail
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
	...
	if(!$stmt->bind_param("sssssi"
						...
						,$p_1_head
						,$p_1_text									
						,$_SESSION['video_library_detail_id'])){
...
$sql = "SELECT *
FROM video_library_detail 
WHERE video_library_detail_id = '".$_SESSION['video_library_detail_id']."'";
$result = $dbCustom->getResult($db,$sql);	
if($result->num_rows > 0){
...
<form name="form" action="video-library-detail.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="video_library_detail_id" value="<?php echo $_SESSION['video_library_detail_id']; ?>">
