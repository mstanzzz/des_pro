...
$page_title = "Editing: account_payments page";
$page_group = "account";
$page = "account";
$ts = time();
...
$sql = "SELECT account_payments_id FROM account_payments WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
$result = $dbCustom->getResult($db,$sql);
if($result->num_rows == 0){
	$sql = "INSERT INTO account_payments
		(profile_account_id) 
		VALUES ('".$_SESSION['profile_account_id']."')"; 
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['account_payments_id'] = $db->insert_id;
}else{
	$_SESSION['account_payments_id'] = (isset($_REQUEST['account_payments_id'])) ? $_REQUEST['account_payments_id'] : 0;
}
if(!is_numeric($_SESSION['account_payments_id'])) $_SESSION['account_payments_id'] = 0;
...
if(isset($_POST['submit'])){
	...
	$stmt = $db->prepare("UPDATE account_payments
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
...
WHERE account_payments_id = '".$_SESSION['account_payments_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<form name="form" action="account-payments.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="account_payments_id" value="<?php echo $_SESSION['account_payments_id']; ?>">
