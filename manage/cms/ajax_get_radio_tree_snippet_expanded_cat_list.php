...
foreach ($t_cats as $t_cat) {
	...
	$max_depth = 5;
		...
		$block .= "<a tabindex='-1' class='tree-parent' onclick='show_children(".$t_cat['cat_id'].")'  data-catid='".$t_cat['cat_id']."' data-cattype='topcat'>
		<img src='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/cart/thumb/".$t_cat['file_name']."' />".stripslashes($t_cat['name'])."</a>";
		
		$checked = ($t_cat['cat_id'] == $_SESSION['temp_cat']['cat_id'])  ? "checked='checked'" : '';
		
		$block .= "<input class='checkbox' onclick='updateOptions(\"".$t_cat['cat_id']."\")' type='radio' id='".$t_cat['cat_id']."' name='cat_id' value='".$t_cat['cat_id']."' ".$checked." />
		<input type='hidden' value='".$t_cat['name']."' name='categoryname' class='categoryname' />";
		$block .= "<ul role='group' class='childrenplaceholder'>".getChildren($t_cat['cat_id'], SITEROOT, $subject_cat_id, $max_depth)."</ul></li>";
}
echo $block;
