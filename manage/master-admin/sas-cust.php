...
//require_once($real_root."/includes/braintree/lib/Braintree.php");
...
if(isset($_POST['edit_account'])){
	...
	//echo "<br /> len cc num    ".strlen($card_num);
	//echo "<br /> numeric cc num    ".is_numeric($card_num);
	//echo "<br />update_billing_name  ".$update_billing_name;
	//echo "<br />update_cc_info  ".$update_cc_info;
	//echo "<br />update_billing_amount  ".$update_billing_amount;
	...
	/* Maybe This should be done with javascript before submit */
...
$result = $dbCustom->getResult($db,$sql);		
	...
	*/
		...
		$path = $real_root."/saas-customers"."/".$d_profile_account_id."/sitemap";	
		...
		$path = $real_root."/saas-customers"."/".$d_profile_account_id."/robots";	
		$files = glob($path."/*"); // get all file names
		foreach($files as $file){ // iterate files
			if(is_file($file)) unlink($file); // delete file			
		}
		...
		$path = $real_root."/saas-customers"."/".$d_profile_account_id;
		$files = glob($path."/*"); // get all file names
		foreach($files as $file){ // iterate files
			if(is_file($file)) unlink($file); // delete file			
		}
					
		if(file_exists($real_root."/saas-customers"."/".$d_profile_account_id)){		
			rmdir($real_root."/saas-customers"."/".$d_profile_account_id);
		}	
