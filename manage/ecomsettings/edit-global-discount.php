...
if(!isset($_SESSION['temp_fields']['hide'])) $_SESSION['temp_fields']['hide'] = $hide;
if(!isset($_SESSION['temp_fields']['when_active'])) $_SESSION['temp_fields']['when_active'] = $when_active;
if(!isset($_SESSION['temp_fields']['when_expired'])) $_SESSION['temp_fields']['when_expired'] = $when_expired;
...
if(!isset($_SESSION['temp_fields']['type'])) $_SESSION['temp_fields']['type'] = $type;
if(!isset($_SESSION['temp_fields']['coupon_code'])) $_SESSION['temp_fields']['coupon_code'] = $coupon_code;
if(!isset($_SESSION['temp_fields']['percent_off'])) $_SESSION['temp_fields']['percent_off'] = $percent_off;
if(!isset($_SESSION['temp_fields']['amount_off'])) $_SESSION['temp_fields']['amount_off'] = $amount_off;
if(!isset($_SESSION['temp_fields']['if_greater_than'])) $_SESSION['temp_fields']['if_greater_than'] = $if_greater_than;
if(!isset($_SESSION['temp_fields']['if_less_than'])) $_SESSION['temp_fields']['if_less_than'] = $if_less_than;
if(!isset($_SESSION['temp_fields']['can_use_with_other_discounts'])) $_SESSION['temp_fields']['can_use_with_other_discounts'] = $can_use_with_other_discounts;
...
$(document).ready(function () {
    ...
    $("#type").change(function () {
        ...
        if ($(this).val().indexOf("coupon") >= 0) {
            htmlstr = "<label>Coupon Code</label><input type='text' name='coupon_code' value='<?php echo $_SESSION['temp_fields']['coupon_code']; ?>' />";
...
<div class="lightboxholder">
	...
	<form name="form" action="<?php echo $url_str; ?>" method="post"  enctype="multipart/form-data" target="_top">
		...
		<div class="lightboxcontent">
			<h2>Edit '<?php echo $_SESSION['temp_fields']['name']; ?>' Discount</h2>
			<fieldset class="colcontainer">
				<legend>Set Current Status</legend>
                <input type="hidden" name="hide" value="<?php echo $_SESSION['temp_fields']['hide']; ?>">
                <!--
				<div class="threecols">
				...
				<div class="threecols">
					...
					<?php $w_d =  ($_SESSION['temp_fields']['when_active'] > 0) ? date("m/d/Y",$_SESSION['temp_fields']['when_active']) : ""; ?>
					<input type="text" name="when_active" id="datepicker1" value="<?php echo $w_d; ?>" />
					&nbsp;<span >12:00am</span> 
                </div>
				<div class="threecols">
					<label>When to expire</label>
					<?php $w_d =  ($_SESSION['temp_fields']['when_expired'] > 0) ? date("m/d/Y",$_SESSION['temp_fields']['when_expired']) : ""; ?>
			...
			<fieldset>
				...
				<div class="colcontainer formcols">
					...
					<div class="twocols">
<!--
<textarea id="wysiwyg" class="wysiwyg" name="description"><?php echo stripslashes($_SESSION['temp_fields']['description']); ?></textarea> 
-->	
<textarea name="description"><?php echo stripslashes($_SESSION['temp_fields']['description']); ?></textarea> 
				...
				<div class="colcontainer">
					<div class="twocols">
						...
						<select id='type' name="type">
							...
							<option value="discount" <?php if($_SESSION['temp_fields']['type'] == "discount") echo "selected" ?>>auto discount all products</option>
							<option value="coupon" <?php if($_SESSION['temp_fields']['type'] == "coupon") echo "selected" ?>>coupon all products</option>
							<option value="discount_closet" <?php if($_SESSION['temp_fields']['type'] == "discount_closet") echo "selected" ?>>auto discount closet only</option>
							<option value="coupon_closet" <?php if($_SESSION['temp_fields']['type'] == "coupon_closet") echo "selected" ?>>coupon closet only</option>
							<option value="discount_accessory" <?php if($_SESSION['temp_fields']['type'] == "discount_accessory") echo "selected" ?>>auto discount accessory only</option>
							<option value="coupon_accessory" <?php if($_SESSION['temp_fields']['type'] == "coupon_accessory") echo "selected" ?>>coupon accessory only</option>
						</select>
					</div>
					<div class="twocols" id="coupon_code_container">
						<?php if($_SESSION['temp_fields']['coupon_code'] != "" || $_SESSION['temp_fields']['type'] == "coupon"){ ?>
						<label>Coupon Code</label>
						<input type="text" name="coupon_code"  value="<?php echo $_SESSION['temp_fields']['coupon_code']; ?>" />
						<?php }else{ ?>
						<input type="hidden" name="coupon_code" value="<?php echo $_SESSION['temp_fields']['coupon_code']; ?>" />
						<?php } ?>
					</div>
				</div>
				<div class="colcontainer">
					<div class="twocols">
						...
						<input type="text" name="percent_off"  value="<?php echo $_SESSION['temp_fields']['percent_off']; ?>" onBlur="checkPercent(this)" 
...
<span class="input_note">Integers bewteen 1 and 100 (cannot use with amount off)</span>
					...
					<div class="twocols">
						...
						<span class="prepend-input">$</span><input type="text" name="amount_off" class="prepended" value="<?php echo $_SESSION['temp_fields']['amount_off']; ?>" 
...
<?php //echo "hide:".$_SESSION['temp_fields']['hide']; ?>

			<fieldset class="colcontainer">
				<legend>Conditions</legend>
					<div class="twocols">
						<label>Apply if <strong>Greater</strong> Than:</label>
						<span class="prepend-input">$</span><input type="text" class="prepended" name="if_greater_than"  value="<?php echo $_SESSION['temp_fields']['if_greater_than']; ?>" 
                        onBlur="checkNum(this)" />
						<span class="input_note">dollar amount</span>
						<label>Apply if <strong>Less</strong> Than:</label>
						<span class="prepend-input">$</span><input type="text" class="prepended" name="if_less_than"  value="<?php echo $_SESSION['temp_fields']['if_less_than']; ?>" 
                        onBlur="checkNum(this)" />
						<span class="input_note">dollar amount</span>
					</div>
					<div class="twocols">
						...
						<?php if($_SESSION['temp_fields']['can_use_with_other_discounts'] > 0) echo "checked"; ?> /></div>
					</div>
			</fieldset>
		</div>
