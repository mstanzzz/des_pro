...
unset($_SESSION['s_f_acces_id']);


if(isset($_POST['add_s_f_acces'])){
	...
	$sql = sprintf("INSERT INTO s_f_acces 
		(title
		,sub_title
		,sub_1_title
		,text
	...
	$s_f_acces_id = $db->insert_id;

}



if(isset($_POST['update_s_f_acces'])){

	$svg_id = (isset($_POST['svg_id']))? $_POST['svg_id'] : 0;
	$s_f_acces_id = (isset($_POST['s_f_acces_id']))? $_POST['s_f_acces_id'] : 0;
	$img_id = (isset($_POST['img_id']))? $_POST['img_id'] : 0;
	...
	$sql = "UPDATE s_f_acces 
			...
			,svg_id = '".$svg_id."'
			,img_id = '".$img_id."'
			WHERE s_f_acces_id = '".$s_f_acces_id."'";
	$res = $dbCustom->getResult($db,$sql);
...
if(isset($_POST['set_active'])){
	...
	//print_r($actives);
	$sql = "UPDATE s_f_acces SET active = '0' 
			WHERE profile_account_id = '".$_SESSION['profile_account_id']."'";
	$result = $dbCustom->getResult($db,$sql);
	foreach($actives as $key => $value){
		$sql = "UPDATE s_f_acces SET active = '1' WHERE s_f_acces_id = '".$value."'";
		$res = $dbCustom->getResult($db,$sql);			
...
if(isset($_POST['del_id'])){
	$s_f_acces_id = $_POST['del_id'];
	$sql = "DELETE FROM s_f_acces 
			WHERE s_f_acces_id = '".$s_f_acces_id."'";
	$result = $dbCustom->getResult($db,$sql);
...
//echo "strip: ".$strip;
	...
	if($sortby != ''){
		...
		$sql .= " ORDER BY s_f_acces_id";					
	}
	$result = $dbCustom->getResult($db,$sql);
	
	$i = 0;
	while($row = $result->fetch_object()) {
		$total_t_cats[$i]['s_f_acces_id'] = $row->s_f_acces_id;
	...
	echo "<a style='margin-left:30px;' class='btn btn-large btn-primary' href='".$url_str."'>Add New</a>"; 
...
<tr style="height:60px; background-color:#ababab; color:white; font-size:1.4em;">
	<td width="10%"><br />Image</td>
	<td>
	<a href="s-f-acces-list.php?sortby=title&a_d=a">
	<img width="30" src="<?php echo SITEROOT."manage/images/up.jpg" ?>"></a>
	<br />
	Title
	<br />
	<a href="s-f-acces-list.php?sortby=title&a_d=d">
	<img width="30" src="<?php echo SITEROOT."manage/images/down.jpg" ?>"></a>		
	</td>
	<td width="10%"><br />SVG</td>
	<td width="10%"></td>
	<td width="10%" align="center"><br />Active</td>
	<td width="10%"></td>
...
foreach($total_t_cats as $t_spft) {
	...
	$url_str .="?s_f_acces_id=".$t_spft['s_f_acces_id']."&for_showroom=1";
	$block .= "<td>";
	...
	$block	.= "<td align='center' valign='middle' >
				...
				value='".$t_spft['s_f_acces_id']."' ".$checked." /></div>";
	$block .= "</td>";	
	
	
				
	$url_str = "edit-s-f-acces.php";
	$url_str .= "?s_f_acces_id=".$t_spft['s_f_acces_id'];
	$url_str .= "&firstload=1";									
	$url_str .= "&pagenum=".$pagenum;
	$url_str .= "&sortby=".$sortby;
	$url_str .= "&a_d=".$a_d;
	...
	$block .= "<td>
				...
				<input type='hidden' id='".$t_spft['s_f_acces_id']."' 
				class='itemId' value='".$t_spft['s_f_acces_id']."' />Delete</a>";
	$block .= "</td>";
	$block .= "</tr>";
}
$block .= "</table>";
