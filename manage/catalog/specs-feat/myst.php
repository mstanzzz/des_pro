...
$module = new Module;
$page_title = "Edit specs_feat";
$page_group = "specs_feat";

$strip = (isset($_GET['strip'])) ? $_GET['strip'] : 0;
$msg = (isset($_GET['msg'])) ? $_GET['msg'] : '';

$specs_feat_id = (isset($_GET['specs_feat_id'])) ? $_GET['specs_feat_id'] : 0;
if($specs_feat_id>0){
	$_SESSION['specs_feat_id'] = $specs_feat_id;
}
...
if(!isset($_SESSION['temp_like_item_array'])){
	$_SESSION['temp_like_item_array'] = array();
	$sql = "SELECT item_id
			FROM specs_feat_to_like_item
			WHERE specs_feat_id = '".$_SESSION['specs_feat_id']."'";
	$result = $dbCustom->getResult($db,$sql);	
	while($row = $result->fetch_object()){
		$_SESSION['temp_like_item_array'][] = $row->item_id;
	}
}

if(!isset($_SESSION['svg_id_array'])){
	$_SESSION['svg_id_array'] = array();
	$sql = "SELECT svg_id
			FROM specs_feat_to_svg
			WHERE specs_feat_id = '".$_SESSION['specs_feat_id']."'";
	$result = $dbCustom->getResult($db,$sql);	
	while($row = $result->fetch_object()){
		$_SESSION['svg_id_array'][] = $row->svg_id;
	}
}

print_r($_SESSION['svg_id_array']);





$sql = "SELECT * FROM specs_feat WHERE specs_feat_id = '".$_SESSION['specs_feat_id']."'"; 
$result = $dbCustom->getResult($db,$sql);
if($result->num_rows > 0){
	...
	$spec_color_1 = $object->spec_color_1;
	$spec_color_2 = $object->spec_color_2;
	$spec_color_3 = $object->spec_color_3;
	$sub_1_img_id = $object->sub_1_img_id;
	$sub_2_img_id = $object->sub_2_img_id;
	$sub_3_img_id = $object->sub_3_img_id;
	$svg_id = $object->svg_id;
	
	$spec_materials = $object->spec_materials;	
	$spec_tips = $object->spec_tips;
	$useful_link_1 = $object->useful_link_1;
	$useful_link_2 = $object->useful_link_2;
	$useful_link_3 = $object->useful_link_3;
...
if(!isset($_SESSION['temp_fields']['spec_color_1'])) $_SESSION['temp_fields']['spec_color_1'] = $spec_color_1;	
if(!isset($_SESSION['temp_fields']['spec_color_2'])) $_SESSION['temp_fields']['spec_color_2'] = $spec_color_2;	
if(!isset($_SESSION['temp_fields']['spec_color_3'])) $_SESSION['temp_fields']['spec_color_3'] = $spec_color_3;	
if(!isset($_SESSION['temp_fields']['spec_materials'])) $_SESSION['temp_fields']['spec_materials'] = $spec_materials;	
if(!isset($_SESSION['temp_fields']['spec_tips'])) $_SESSION['temp_fields']['spec_tips'] = $spec_tips;	
if(!isset($_SESSION['temp_fields']['useful_link_1'])) $_SESSION['temp_fields']['useful_link_1'] = $useful_link_1;	
if(!isset($_SESSION['temp_fields']['useful_link_2'])) $_SESSION['temp_fields']['useful_link_2'] = $useful_link_2;	
if(!isset($_SESSION['temp_fields']['useful_link_3'])) $_SESSION['temp_fields']['useful_link_3'] = $useful_link_3;	
...
$_SESSION['temp_fields']['short_description'] = $short_description;	
...
<form name="form1" action="<?php echo $url_str; ?>" method="post" onSubmit="return validate(this)"  enctype="multipart/form-data" target="_top">
	<input type="hidden" name="specs_feat_id" value="<?php echo $_SESSION['specs_feat_id']; ?>" />
    <input type="hidden" name="update_specs_feat" value="1">  
...
$url_str .= "&img_type=sub_1_img_id";
?>
<a onClick="set_cat_session();" style="color:#c3fdf5; padding:16px; font:bold; font-size: 18px;" 
...
//print_r($_SESSION['temp_like_item_array']);
