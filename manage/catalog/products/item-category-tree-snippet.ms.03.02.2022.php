...
foreach ($t_cats as $t_cat) {
	...
	$block .= "data-catid='".$t_cat['cat_id']."' data-cattype='topcat'>";
	$block .= "<img src='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/cart/tiny/".$t_cat['file_name']."' />".$t_cat['name'].'';
	$checked = inArray($t_cat['cat_id'], $_SESSION['temp_item_cats'], 'cat_id') ? "checked='checked'" : '';
	$block .= "<input class='checkbox' onclick='updateOptions(".$t_cat['cat_id'].")' ";
	$block .= " type='checkbox' id='".$t_cat['cat_id']."' value='".$t_cat['cat_id']."' ".$checked." />";
	$block .= "<input type='hidden' value='".$t_cat['name']."' name='categoryname' class='categoryname' /></a>";
