...
if(!isset($real_root)){
	if(strpos($_SERVER['REQUEST_URI'], 'test' )){
		$real_root = $_SERVER['DOCUMENT_ROOT'].'/test';
	}elseif(strpos($_SERVER['REQUEST_URI'], 'solvitware/' )){  
	...
	if($_SESSION['temp_fields']['parent_item_id'] == 0){
		if(count($attr_array) > 0){
			...
			foreach($attr_array as $val){	
				$sel = ($val['attribute_id'] == $_SESSION['temp_fields']['main_attr_id'])? 'selected' : '';				
		...
		$block .= "<div class='twocols'>";
		$main_attr_name = get_main_attr_name($_SESSION['temp_fields']['main_attr_id'], $dbCustom);
