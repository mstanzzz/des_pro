...
if(strpos($_SESSION['img_type'], 'art_vide') !== false){
	...
	if($_SESSION['ret_page']=='add-video'){
		$url_str = SITEROOT."manage/catalog/video/add-video.php"; 
	}else{
		$url_str = SITEROOT."manage/catalog/video/edit-video.php"; 
	}
...
if(strpos($_SESSION['img_type'], 'pec_sizes_') !== false){
	$db = $dbCustom->getDbConnect(CART_N_DATABASE);
	$sql = "INSERT INTO image (file_name, profile_account_id) 
			VALUES ('".$orig_img_fn."', '".$_SESSION['profile_account_id']."')";
	$r = $dbCustom->getResult($db,$sql);
	...
	if(strpos($_SESSION['img_type'], 'pec_sizes_1_img') !== false){
		$_SESSION['temp_cat_fields']['spec_sizes_1_img_id'] = $id;
		$sql = "UPDATE category
				SET spec_sizes_1_img_id = '".$id."'
	...
	}elseif(strpos($_SESSION['img_type'], 'pec_sizes_2_img') !== false){
		$_SESSION['temp_cat_fields']['spec_sizes_2_img_id'] = $id;
		$sql = "UPDATE category
				SET spec_sizes_2_img_id = '".$id."'
				WHERE cat_id = '".$_SESSION['cat_id']."'";
		$r = $dbCustom->getResult($db,$sql);
		$new_path_fn = "../saascustuploads/".$_SESSION['profile_account_id']."/cart/".$orig_img_fn;
		$dst_w = 240;
		...
		$_SESSION['temp_cat_fields']['spec_sizes_3_img_id'] = $id;
		$sql = "UPDATE category
				SET spec_sizes_3_img_id = '".$id."'
				WHERE cat_id = '".$_SESSION['cat_id']."'";
		$r = $dbCustom->getResult($db,$sql);
		$new_path_fn = "../saascustuploads/".$_SESSION['profile_account_id']."/cart/".$orig_img_fn;
		$dst_w = 240;
...
$gallery++;
	...
	$_SESSION['temp_gallery'][] = $_SESSION['gal_img_id'];
...
if(strpos($_SESSION['img_type'], 'tep_galler') !== false){
...
if(strpos($_SESSION['img_type'], 'cart') !== false || strpos($_SESSION['img_type'], 'ain') !== false){
	...
	if($_SESSION['crop_n'] < 2){
		...
		$new_path_fn = "../saascustuploads/".$_SESSION['profile_account_id']."/cart/full/".$orig_img_fn;
		$dst_w = 620;
		$dst_h = 620;
		$dst_img = imageCreateTrueColor($dst_w,$dst_h);
		imagecopyresampled($dst_img, $canvas, 0, 0, 0, 0, $dst_w, $dst_h, $src_w, $src_h);
