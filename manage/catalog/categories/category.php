...
if(isset($_POST['add_cat'])){
	...
	if(1){
		...
		if($img_res->num_rows > 0){
			...
			$old_img_file_name = $img_obj->file_name;
			$pos = strlen($old_img_file_name) - 3;
			$ext = substr($old_img_file_name,$pos);

			$new_img_file_name = getUrlText($name.'-'.$img_id);	
			if(strlen($new_img_file_name) > 100){
				$new_img_file_name = cut_name_from_front($new_img_file_name, 100);
			}
			
			$new_img_file_name .= '.'.$ext;

			rename_cart_images_in_dirs($old_img_file_name, $new_img_file_name, $real_root, $domain);	
			update_cart_image_file_name_in_db($img_id, $new_img_file_name);			
		}
		*/
...
if(!$strip){ 
	...
	$result = $dbCustom->getResult($db,$sql);
	while($row = $result->fetch_object()){
		$canonical_part = SITEROOT."/".$_SESSION['global_url_word'].$row->seo_url;
		$sql = "UPDATE category 
...
<div class="manage_page_container <?php if($strip){ echo "lightbox"; }?>">
	...
	<div class="manage_main">
		...
		if(!$strip){
			...
			$admin_bread_crumb->add($label, SITEROOT.$_SERVER['REQUEST_URI']);
		...
		if($search_str != ''){
			if(is_numeric($search_str)){
				$sql .= " AND (category.name like '%".$search_str."%' || category.profile_cat_id = '".$search_str."')";
			}else{
    	...
    	<form name="form" action="<?php echo $url_str; ?>" method="post" enctype="multipart/form-data">
			...
			<div class="data_table">
                ...
                if($total_rows > $rows_per_page){
					echo getPagination($total_rows, $rows_per_page, $pagenum, $truncate, $last, "catalog/categories/category.php", $sortby, $a_d,0,0, $search_str,0,0,$strip); 
					echo "<br /><br /><br /><br />";
				}
				?>
				...
				<table cellpadding="10" cellspacing="0">
					...
					foreach ($sub_cats as $sub_cat) {
						...
						if(!$strip){
							$block .= "<a class='fancybox' href='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/tmp/pre-crop/".$sub_cat['file_name']."'>";
							$block .= "<img  src='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/cart/thumb/".$sub_cat['file_name']."'></a></td>";
						}else{
							$block .= "<a  href='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/cart/full/".$sub_cat['file_name']."'>";
							$block .= "<img  src='".SITEROOT."/saascustuploads/".$_SESSION['profile_account_id']."/cart/thumb/".$sub_cat['file_name']."'></a>";
						}
