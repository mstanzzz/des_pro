...
class ShoppingCart {
	...
	function __construct($dbCustom){
	   $this->customer_id = (isset($_SESSION['ctg_cust_id']))? 
	   $_SESSION['ctg_cust_id'] : 0;
	   $this->anonymous_shopper_id = (isset($_SESSION['anonymous_shopper_id']))? 
	   $_SESSION['anonymous_shopper_id'] :rand();	
		...
		if($load){
			...
			while($row = $result->fetch_object()) {
				...
				if($res->num_rows > 0){
					...
					$_SESSION['cart'][$i]['name'] = stripslashes($object->name);
					...
					$_SESSION['cart'][$i]['short_description'] = stripslashes($object->short_description);
					$i++;
				}
			}
			$this->cart_array = $_SESSION['cart'];			
	...
	function addItem($dbCustom,$item_id, $qty = 1)
	{
		if($this->itemInCart($item_id)){
			...
			if($result->num_rows > 0){
				...
				$_SESSION['cart'][$i]['is_free_shipping'] = $object->is_free_shipping;
				$_SESSION['cart'][$i]['is_drop_shipped'] = $object->is_drop_shipped;	
				$_SESSION['cart'][$i]['short_description'] = $object->short_description;	
