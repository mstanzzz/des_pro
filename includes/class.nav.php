...
class Nav {
	...
	function getSeoUrl($slug=''){
		$url_str = '';
		return $url_str;
	}
	
	function loadPages($dbCustom)
	{
	...
	function getNavCats($dbCustom,$show_in='showroom')
	{		
		$cats_array = array();	
		...
		$i = 0;
		while($row = $result->fetch_object()) {
			$cats_array[$i]['cat_id'] = $row->cat_id;
			if(strlen($row->name) < 40){
				$name = stripslashes($row->name);
			}else{
				$name = stripslashes($row->short_name);
			}
			$cats_array[$i]['name'] = $name;
			$cats_array[$i]['url'] = $this->getCatUrl($name, $row->cat_id, $show_in);				
			$i++;	
		}
		return $cats_array;
	}
	...
	function getCatUrl($name, $cat_id, $show_in = 'showroom'){
		...
		if($show_in == 'showroom'){			
			...
			$url_str = SITEROOT.'accessory-products-'.$cat_id.'/'.$this->getUrlText($name).'.html';
		}
		*/
		$url_str = SITEROOT.'showroom-category-'.$cat_id.'/'.$this->getUrlText($name).'.html';
